...
class BaseConfig(BaseModel):
    ...
    hyperparameters: list[HyperParameter] = Field(
        ...
        description="List of hyperparameters for this component"
    )
...
class HerculeConfig(BaseModel):
    ...
    models: list[ModelConfig] = Field(
        default_factory=list,
        description="List of models to test with their configurations"
    )
    max_iterations: int = Field(
        default=1000, 
        ge=1, 
        description="Maximum number of learning iterations"
    )
    output_dir: Path = Field(
        default=Path("outputs"), 
        description="Directory for saving results and models"
    )
